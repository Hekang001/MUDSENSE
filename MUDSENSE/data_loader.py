...
import os
from tool.data_process import load_data_process_file
import utils
import requests
    ...
    for index, instance in enumerate(data):
        ...
        for entity in instance["ner"]:
            ...
            for i in range(len(index)):
                if i + 1 >= len(index):
                    break               
                if index[i] < length and index[i + 1] < length:
                    _grid_labels[index[i], index[i + 1]] = 1
                else:
                    print(f"Warning: Index out of bounds. length: {length}, index[i]: {index[i]}, index[i + 1]: {index[i + 1]}")
...
def load_data_bert(config):
    ...
    with open('./MUDSENSE/data/{}/train.json'.format(config.dataset), 'r', encoding='utf-8') as f:
        train_data = json.load(f)
    with open('./MUDSENSE/data/{}/dev.json'.format(config.dataset), 'r', encoding='utf-8') as f:
        dev_data = json.load(f)
    with open('./MUDSENSE/data/{}/test.json'.format(config.dataset), 'r', encoding='utf-8') as f:
        test_data = json.load(f)
    ...
    tokenizer = AutoTokenizer.from_pretrained("./MUDSENSE/model/bert-base-chinese")
...
# 读取sense文件
def inference_load_data_bert(inference_path, config):
    ...
    with open("./MUDSENSE/inference/inference_input.json",'w',encoding='utf-8') as f:
        json.dump(result, f, ensure_ascii=False, indent=4)

    vocab = Vocabulary()

    table = pt.PrettyTable([config.dataset, 'sentences'])
    table.add_row(['Inference', len(result)])
    # config.logger.info("\n Inference数据集规模：\n{}".format(table))

    with open("./MUDSENSE/inference/inference_input.json",'r',encoding='utf-8') as f:
        data = json.load(f)

    predict_dataset = RelationDataset(*process_bert(data, tokenizer, vocab))
    return predict_dataset, result



def inference_single_load_data_bert(sentence, config):

    tokenizer = AutoTokenizer.from_pretrained("model/roberta-base")
    vocab = Vocabulary()
    result = []
    words = sentence.strip().split()
    result.append({"sentence": words,"ner":[]})

    predict_dataset = RelationDataset(*process_bert(result, tokenizer, vocab))

    # print(RelationDataset(predict_dataset))
    print(result)
    return predict_dataset, result
