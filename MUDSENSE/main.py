    ...
    def predict(self, epoch, data_loader, data):
        ...
        with torch.no_grad():
            for data_batch in data_loader:
                ...
                #     for ent in ent_list:
                #         instance["predict_entity"].append({"text": [sentence_1[x] for x in ent[0]],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--predict_path', type=str, default='./senses_result.json')
    ...
    for i in range(config.epochs):
        ...
        if test_f1 > best_test_f1:
            best_test_f1 = test_f1
    logger.info("Best DEV Precision: {:3.4f}".format(best_f1))
    logger.info("Best TEST Precision: {:3.4f}".format(best_test_f1))
    # print(config.save_path)
    # print(config.predict_path)
    logger.info("save-path:{}".format(config.save_path))
    logger.info("predict-path:{}".format(config.predict_path))
